...
const HowItWorks = ({ translations, lang }: HowItWorksProps) => {
  ...
  const steps = translations.steps.map((step: any, index: number) => ({
    ...step,
    color: ["bg-blue-500", "bg-indigo-500", "bg-purple-500", "bg-pink-500"][
      index
    ],
  }));
          ...
          <div className="space-y-12 md:space-y-0 relative z-10">
            {steps.map((step: any, index: any) => (
              <div
                ...
                } md:items-center opacity-0 transition-all duration-1000 delay-${
                  index * 200
                }`}
              >
                {/* Content */}
                <div className="md:w-1/2 p-6">
                  <div
                    className={`inline-flex items-center justify-center w-12 h-12 rounded-full ${step.color} text-white font-bold mb-4 shadow-lg`}
                  >
                    {step.number}
                  </div>
                ...
                <div className="hidden md:flex items-center justify-center md:w-24 relative z-10">
                  <div
                    className={`w-8 h-8 rounded-full border-4 border-white ${step.color} shadow-md`}
                  ></div>
                </div>
